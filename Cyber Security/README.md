🔐 Cyber Security - Projeto Acadêmico



Projeto acadêmico desenvolvido para a disciplina de Cyber Security da UniFECAF.



## 🎯 Objetivo



O objetivo deste trabalho foi aplicar conceitos básicos de segurança da informação utilizando ferramentas de análise, monitoramento e varredura de rede.



---



## 🛠️ Ferramentas Utilizadas



* Wireshark

* Nmap

* VirusTotal

* Prompt de Comando (CMD)

* Windows 10



---



## 🔎 Atividades Realizadas



### ✅ Captura de pacotes com Wireshark



* Monitoramento de tráfego DNS

* Captura de pacotes TCP

* Análise de tráfego HTTP


### ✅ Varredura de portas com Nmap



* Escaneamento da máquina local

* Identificação de portas abertas

* Análise de serviços ativos


### ✅ Análise de URLs no VirusTotal

* Verificação de reputação

* Análise de ameaças

* Consulta de segurança online

---

## 📂 Estrutura do Projeto

bash

Cyber Security/

│

├── README.md

├── Relatório Explicativo-Cyber Security.pdf

├── Relatório Explicativo-Cyber Security.docx

├── docs/

├── imagens/

---

## 📸 Evidências

As evidências das análises realizadas estão disponíveis na pasta:

bash

imagens

---

## 🎓 Instituição



UniFECAF – Engenharia da Computação

---

## 👨‍💻 Autor

Adriano Mantoan

Técnico em Segurança do Trabalho | Estudante de Engenharia da Computação
